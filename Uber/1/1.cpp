...
void DepthFirstSearch(std::vector<std::vector<int>> Grid, int row, int col,
	...
	for (int k = 0; k < 4; ++k) {
		if (isPointWithinGrid(Grid, row + rowDir[k], col + colDir[k],
				      visited, RowSize, ColSize)) {
			count++;
			DepthFirstSearch(Grid, row + rowDir[k], col + colDir[k],
					 visited, count, RowSize, ColSize);
		}
	}
}
	...
	for (int i = 0; i < RowSize; ++i) {
		for (int j = 0; j < ColSize; ++j) {
			if (Grid[i][j] && !visited[i][j]) {
				int count = 1;
				DepthFirstSearch(Grid, i, j, visited, count,
						 RowSize, ColSize);
